...
if which rvm-prompt &> /dev/null; then
  RPROMPT='%{reset_color%}${time} %{$reset_color%} %{$fg[040]%}$(~/.rvm/bin/rvm-prompt i v g) %{$reset_color%}'
else
  if which rbenv &> /dev/null; then
    RPROMPT='%{$reset_color%}${time} %{$reset_color%} %B %{$FG[105]%} $(rbenv version | sed -e "s/ (set.*$//") %{$reset_color%} %b'
  else
    RPROMPT='%{$reset_color%}${time}'
  fi
fi

time_enabled="%(?.%{$fg_bold[blue]%}.%{$fg[040]%})%*%{$reset_color%}"
